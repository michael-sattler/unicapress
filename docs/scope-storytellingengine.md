...
**Related docs:** [world-content-model-v1.md](./world-content-model-v1.md) (entity schema) · [workplan.md](./workplan.md) (phased build plan)
...
3. Every telling is generated exactly once, fixed scene-by-scene as read, and never regenerable. Fixed text is durable and re-readable forever.
...
**Note on scope:** this PRD describes **the App** — the three planes below. A separate system, **the Shell** (marketing site + staff admin/ops console, PHP/mysqli on cPanel), is scoped independently in [scope-marketing-shell.md](./scope-marketing-shell.md) and is being built first. The Shell and the App share no database, session, or auth; see [standards-architecture+deployment.md](./standards-architecture+deployment.md) for the boundary.
...
## 6. World Content Model (summary)

The World Package is a **versioned bundle** of typed entities (locations, characters, objects, themes, organizations, events, relationships, attributes) plus **compiled packets** derived from them for model context. Entity shapes adapt the Amanuensis *Content and structure* reference ([scope-amanuensis.md](./scope-amanuensis.md)); workflows do not — Unica Press is a publishing engine, not a writer's studio.

**Full schema, field definitions, and Amanuensis carry-over/exclude tables:** [world-content-model-v1.md](./world-content-model-v1.md).

**Binding rules (PRD-level):**

| Rule | Detail |
|---|---|
| Canon tiers | `draft` → authoring; `canon` → published package; `excluded` → withheld from generation (A1.8) |
| Apocryphal tellings | Generation-plane output never writes back; author canonizes explicitly (F1.5) |
| Entities → packets | Authoring source of truth compiles to token-budgeted invariants, location, and optional entity slices |
| `*_meta` | AI-generated synopsis from description + attributes; author-editable; v1 auto-regen always overwrites |
| Cast linking | Composed characters link to canon only when author-deemed canon |
| Objects | Canon types; `parent_object_id` for specific instances; per-telling instances in continuity ledger |
| Skeletons | Ordered **storybeats** with role slots — authored shape, not reader-reorderable plot |
| v1 seed data | Steamlands sourcebook ported into entities (A1.13); greenfield codebase |

---

## 7. FEATURE SPACE A — The Narrative Generation Engine *(core value; world-agnostic)*

### A1. World Package
The complete, versioned, machine-consumable definition of a world. Replaces "prompt engineering" with authored, structured context assembled per scene. Entity types per [world-content-model-v1.md](./world-content-model-v1.md).
...
| A1.2 | **Location packets:** one per gazetteer entry (city + immediate region): geography, character, organizations, sensory palette, local idiom. Compiled from Location entities. Target ≤ 2k tokens each. | P0 |
...
| A1.6 | **Canon ledger:** authoritative structured fact base — entities, relationships, events — queryable by the editorial battery. Tellings cannot write to it. | P0 |
| A1.7 | Packages are **versioned**; every telling records the package version it was prepared under. | P0 |
| A1.8 | **Compartmentalization:** plot-sensitive canon (e.g., Within Wheels material, the Society) flagged `excluded`; excluded material omitted from all packets and queries. | P0 |
| A1.9 | Every published entity has stable id and `*_meta`; auto-generated on source-field save, author-editable, auto-generation overwrites in v1. | P0 |
| A1.10 | **Entity packets:** optional ≤ 1k-token slices for characters, organizations, objects, events — included only when referenced by spine or continuity ledger. | P1 |
| A1.11 | Canon ledger supports structured query by entity id, relationship type, and location — not bulk prompt injection. | P0 |
| A1.12 | Package **publish** compiles packets, runs validation lint, and records content hash per packet. | P0 |
| A1.13 | v1 world content seeded by **porting** the Steamlands sourcebook into world entities (not greenfield prose). | P0 |

### A2. Skeleton Library (story shapes)
Authored, reusable narrative spines composed of **storybeats**. The engine never invents story *shape*; it instantiates authored shapes.

| ID | Requirement | Priority |
|---|---|---|
| A2.1 | **Skeleton spec format:** ordered storybeats with intent descriptions; character role slots (constraint notes, not names); reversal and ending definitions; thematic guardrails and invariants (e.g., "the span is sound"-class rules); scene-count range; register, location, and theme compatibility tags. | P0 |
...
| ID | Requirement | Priority |
|---|---|---|
| A3.1 | On request: derive a **seed** from (request params + patron salt + nonce); select skeleton from compatible set; compose the full telling spine — instantiated characters (named via naming grammar), concrete beats, the ending — in a single composition pass before any scene is manifested. Optional canon link only for author-deemed canon characters. | P0 |
...
| A4.3 | Context assembly per scene: invariants + location packet + optional entity slices (A1.10) + spine + **continuity ledger** (time, knowledge, physical state, objects in play, scene summaries, open threads). | P0 |
| A4.4 | Continuity ledger updated by an extraction pass after each scene is accepted into the buffer. | P0 |
...
| ID | Requirement | Priority |
|---|---|---|
| A5.1 | **Canon check (HoleFinder):** scene claims validated against canon ledger (entities + relationships + events) and continuity ledger; contradictions → regenerate. | P0 |
...
| A8.2 | Quality events (flags, degradations, drop-offs) flow to the author dashboard (Feature Space F). | P1 |

---

## 8. FEATURE SPACE B — The Retrieval Engine Experience *(the interface; Steamlands skin)*
...
### B2. The Retrieval (theater; driven by A8 telemetry)
| ID | Requirement | Priority |
|---|---|---|
...
## 9. Contract Between Engine and Experience (Telling API)
...
## 10. FEATURE SPACE C — The Manuscript (reading experience)
...
## 11. FEATURE SPACE D — Patron & Collection
...
## 12. FEATURE SPACE E — Lending & Acquisition
...
## 13. FEATURE SPACE F — Worldbuilder Tooling (internal, v1)

Minimum viable authoring surface for P2. Greenfield codebase (Amanuensis schema as reference only; no Scribe, Fragment, or Conversation tables). Entity model: [world-content-model-v1.md](./world-content-model-v1.md).

| ID | Requirement | Priority |
|---|---|---|
| F1.1 | **World package editor:** CRUD for world entities; attribute editor; publish compiles packets (A1.12) and bumps immutable package version. | P0 |
| F1.2 | **Skeleton authoring:** storybeat editor with role slots and compatibility tags; validation lint against A2.1; storybeat reorder within skeleton only. | P0 |
...
| F1.5 | **Canonization workflow:** author may promote reviewed apocryphal material into `draft` world entities; changelog on commit; nothing automatic. | P1 |
| F1.6 | **Meta generation:** auto-summarize into `*_meta` on source-field save + explicit Regenerate; UI warns that saving description refreshes meta. | P0 |
| F1.7 | **Image attach:** reference gallery per entity; gazetteer card required for visible gazetteer locations. | P0 |
| F1.8 | **Changelog:** entity field before/after on every save and publish. | P0 |
...
**In:** Feature Space A at P0 (text only — plates may slip); B1, B2 at P0; C1, C2.1, C2.4, C3.2; D1, D2.1; E1, E2.1; F1.1–F1.3, F1.6–F1.8. One world, **4 cities**, **3 registers**, **8 skeletons**, telling length 5–8 scenes.

**Out:** plates, gazetteer unlocks, borrower's card, standing-order compression, print-on-demand, audio, second world, public authoring, entity packet slicing beyond locations unless proofing bench requires it.

**Success test:** ≥ 50% of cold first-time visitors who punch a slip finish their telling; ≥ 25% of finishers lend or start a second telling.

**Build sequencing:** [workplan.md](./workplan.md).
...
1. **Prose quality ceiling** — skeleton-constrained generation, style fingerprint + scoring, editorial battery, proofing bench, MVP gate (§14).
2. **Cost per telling** — per-skeleton cost telemetry, buffer-paced generation, model-tiering for extraction/checks.
3. **Canon drift** — tellings sandboxed (Principle 8), read-only canon ledger, package versioning.
4. **Skeuomorph fatigue** — Principle 4 and B2.6; watch return-visit drop-off.
5. **Cultural/AI backlash** — total disclosure (Principle 3, E2.1); position as publisher of a new form.
6. **Trademark (Unica/HCL)** — clearance search before public naming.
7. **Single-author bottleneck** — F-space tooling maximizes leverage at v1 scale.
8. **Meta overwrite surprise** — F1.6 UI warning; defer author-lock post-MVP.

---

## 17. Open Questions

1. Telling length & pricing model (leaning: first telling free, patronage subscription).
2. Sourcebook sensitive-material boundary — excluded vs. texture (A1.8 manifest needed).
3. Plate count economics — MVP gate or fast-follow?
4. Mobile-first vs. desktop-first faceplate for launch.
5. Anonymous shelf expiry for abandoned tellings.
6. Name/registration: unicapress.com, trademark counsel.
7. Meta author-lock — when to preserve author-edited `*_meta` across auto-regeneration.
